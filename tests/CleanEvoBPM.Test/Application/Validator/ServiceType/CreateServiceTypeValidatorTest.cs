using CleanEvoBPM.Application.CQRS.ServiceType.Command;
using CleanEvoBPM.Application.Validator.ServiceType;
using CleanEvoBPM.Test.Application.Constants;
using FluentValidation.TestHelper;
using System.Linq;
using Xunit;

namespace CleanEvoBPM.Test.Application.Validator.ServiceType
{
    public class CreateServiceTypeValidatorTest
    {
        private CreateServiceTypeCommandValidator _validator;

        public CreateServiceTypeValidatorTest()
        {
            _validator = new CreateServiceTypeCommandValidator();
        }

        [Theory]
        [MemberData(nameof(CommandWithNameEmpty))]
        public void CreateServiceTypeValidator_Failure_NameEmpty(CreateServiceTypeCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ServiceTypeName);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ServiceTypeName" &&
                x.ErrorCode == FluentValidationErrorCode.NotEmptyValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandWithNameTooShort))]
        public void CreateServiceTypeValidator_Failure_NameTooShort(CreateServiceTypeCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ServiceTypeName);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ServiceTypeName" &&
                x.ErrorCode == FluentValidationErrorCode.MinimumLengthValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandWithNameTooLong))]
        public void CreateServiceTypeValidator_Failure_NameTooLong(CreateServiceTypeCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ServiceTypeName);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ServiceTypeName" &&
                x.ErrorCode == FluentValidationErrorCode.MaximumLengthValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandWithDescriptionTooLong))]
        public void CreateServiceTypeValidator_Failure_DescriptionTooLong(CreateServiceTypeCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.Description);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "Description" &&
                x.ErrorCode == FluentValidationErrorCode.MaximumLengthValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandValid))]
        public void CreateServiceTypeValidator_Success(CreateServiceTypeCommand command)
        {
            var result = _validator.TestValidate(command);
            Assert.True(result.IsValid);
        }

        public static TheoryData<CreateServiceTypeCommand> CommandWithNameEmpty = new TheoryData<CreateServiceTypeCommand>
        {
            new CreateServiceTypeCommand(),
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "",
            }
        };

        public static TheoryData<CreateServiceTypeCommand> CommandWithNameTooShort = new TheoryData<CreateServiceTypeCommand>
        {
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "a",
            },
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "abcd",
            }
        };

        public static TheoryData<CreateServiceTypeCommand> CommandWithNameTooLong = new TheoryData<CreateServiceTypeCommand>
        {
            new CreateServiceTypeCommand
            {
                ServiceTypeName = @"1rQrLyL4EkLWsTrgeusWRgpcMZXfyYkRzdJyVKFgbQjh07gfr4
                                    euDtWkfWEgovQU3oNHYLO4FIxf6BDBDWaXtw7Z88cZ8bk4UZ1mz",
            }
        };

        public static TheoryData<CreateServiceTypeCommand> CommandWithDescriptionTooLong = new TheoryData<CreateServiceTypeCommand>
        {
            new CreateServiceTypeCommand
            {
                Description = @"
                                72Jq3zDRLpcPnTLvnCrBEfDRs5CDVhV4PRna"
            }
        };

        public static TheoryData<CreateServiceTypeCommand> CommandValid = new TheoryData<CreateServiceTypeCommand>
        {
            // name length 5
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "ABCDE"
            },
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "ABCDE",
                Description = "Description"
            },
            // name length 100
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "v1dTTYBcz4e4644EzKQOqKMqHBkHznKzrtNLfjx4kObgBkfj92aJ1ZYGaQsVMVphra158KxV014NHbkQaY66toA9wpDiT9JLjsDH",
            },
            // description length 1000
            new CreateServiceTypeCommand
            {
                ServiceTypeName = "Some name",
                Description = "",
            }
        };
    }
}
