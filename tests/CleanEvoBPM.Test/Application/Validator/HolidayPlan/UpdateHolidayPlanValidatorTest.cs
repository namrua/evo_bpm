using CleanEvoBPM.Application.CQRS.ProjectHolidayPlan.Command;
using CleanEvoBPM.Application.Validator.HolidayPlan;
using CleanEvoBPM.Test.Application.Constants;
using FluentValidation.TestHelper;
using System;
using System.Linq;
using Xunit;

namespace CleanEvoBPM.Test.Application.Validator.HolidayPlan
{
    public class UpdateHolidayPlanValidatorTest
    {
        private UpdateProjectHolidayPlanCommandValidator _validator;

        public UpdateHolidayPlanValidatorTest()
        {
            _validator = new UpdateProjectHolidayPlanCommandValidator();
        }

        [Theory]
        [MemberData(nameof(CommandResourceEmpty))]
        public void UpdatePlanValidator_Failure_ResourceEmpty(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ResourceId);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ResourceId" &&
                x.ErrorCode == FluentValidationErrorCode.NotEmptyValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandRoleEmpty))]
        public void UpdatePlanValidator_Failure_RoleEmpty(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ResourceRoleId);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ResourceRoleId" &&
                x.ErrorCode == FluentValidationErrorCode.NotEmptyValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandFromDateEmpty))]
        public void UpdatePlanValidator_Failure_FromDateEmpty(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.FromDate);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "FromDate");
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandToDateEmpty))]
        public void UpdatePlanValidator_Failure_ToDateEmpty(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ToDate);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ToDate");
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandNoteTooLong))]
        public void UpdatePlanValidator_Failure_NoteTooLong(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.Note);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "Note" &&
                x.ErrorCode == FluentValidationErrorCode.MaximumLengthValidator);
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandToDateSmallerThanFromDate))]
        public void UpdatePlanValidator_Failure_ToDateSmallerThanFromDate(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            result.ShouldHaveValidationErrorFor(t => t.ToDate);

            var error = result.Errors.FirstOrDefault(x =>
                x.PropertyName == "ToDate" && x.ErrorCode == "GreaterThanOrEqualValidator");
            Assert.NotNull(error);
        }

        [Theory]
        [MemberData(nameof(CommandValid))]
        public void UpdatePlanValidator_Success(UpdateProjectHolidayPlanCommand command)
        {
            var result = _validator.TestValidate(command);
            Assert.True(result.IsValid);
        }

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandResourceEmpty = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand()
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandRoleEmpty = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand()
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandFromDateEmpty = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand()
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandToDateEmpty = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand()
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandToDateSmallerThanFromDate = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand
            {
                ResourceId = Guid.NewGuid(),
                ResourceRoleId = Guid.NewGuid(),
                FromDate = DateTime.Now,
                ToDate = DateTime.Now.AddMilliseconds(-1)
            }
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandNoteTooLong = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand
            {
                Note = @"
                        72Jq3zDRLpcPnTLvnCrBEfDRs5CDVhV4PRna"
            }
        };

        public static TheoryData<UpdateProjectHolidayPlanCommand> CommandValid = new TheoryData<UpdateProjectHolidayPlanCommand>
        {
            new UpdateProjectHolidayPlanCommand
            {
                ResourceId = Guid.NewGuid(),
                ResourceRoleId = Guid.NewGuid(),
                FromDate = DateTime.Now,
                ToDate = DateTime.Now,
            },
            new UpdateProjectHolidayPlanCommand
            {
                ResourceId = Guid.NewGuid(),
                ResourceRoleId = Guid.NewGuid(),
                FromDate = DateTime.Now,
                ToDate = DateTime.Now.AddMilliseconds(1),
                // note length 1000
                Note = "",
            }
        };
    }
}
